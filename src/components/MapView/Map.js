...
export default function MapView() {
  ...
  useEffect(() => {
    if (updateMap) setCentre(location);
  }, [location, updateMap]);
    ...
    <>
      ...
      <div className="map-controls">
        <button
          ...
          style={
            updateMap
              ? { boxShadow: "inset 0 4px 10px -6px black" }
              : { boxShadow: "0 4px 10px -6px black" }
          }
        >
          <img src={update} width={15} alt="update" />
        </button>
      </div>
      <GoogleMap
        center={centre}
      ...
      >
        ...
        {mapData.map((path, i) => {
          return <Polygon key={`poly${i}`} paths={path} options={options(i)} />;
        })}

        <Marker icon={currentPositionIcon} position={location}></Marker>
      </GoogleMap>
    </>
  );
}
