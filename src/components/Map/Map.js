...
import "./Map.css";
import Street from "../Deliveries/Place";
...
export default function MapView() {
  ...
  const centre = useMemo(
    () => (!location.error ? location : fallbackPosition),
    []
  );
  const { closest } = useSortedDelivery();
  const [highlighted, setHighlighted] = useState(closest.id);
  ...
  return (
    <Flex flexDir="column" w="100%">
      ...
      >
        ...
        <Markers setHighlighted={setHighlighted} />
        <Suburbs />
      </GoogleMap>
...
const Markers = ({ setHighlighted }) => {
  const { selected: streets } = useDeliveryDb();

  const onClick = (e) => {
    for (let i = 0; i < streets.length; i++) {
      if (
        streets[i].lat === e.latLng.lat() &&
